...
import { apiCreateBanner } from "@/services/banner.services";
...
export const CreateBannerModal = () => {
  ...
  const isModalOpen = isOpen && type === "createBanner";
  const { setSelectedPage, addBanner } = useSelectedPageContext();
  ...
  const onSubmit = async (values: z.infer<typeof bannerSchema>) => {
    try {
      ...
      const res = await apiCreateBanner(formData);
      if (res?.data) {
      ...
      const errorMessage =
        error.response?.data?.message || "Lỗi khi tạo banner";
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Select
                        ...
                        onChange={(option) =>
                          field.onChange(option?.value || "ALL_PAGE")
                        }
