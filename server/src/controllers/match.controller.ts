...
export const createMatch: RequestHandler[] = [
  upload.fields([
    ...
    { name: "streamLinkVideos", maxCount: 10 },
    ...
    try {
      ...
      if (body.streamLinks && typeof body.streamLinks === "string") {
        try {
          ...
          res.status(400).json({ message: "Danh sách streamLinks không hợp lệ" });
          return;
        }
      }
      ...
      const streamLinkVideos = files?.streamLinkVideos || [];
      ...
      const processedStreamLinks = streamLinks.map((link: any, index: number) => {
        if (!link.label || !link.url) {
          throw new Error("URL và nhãn stream link là bắt buộc");
        }
        return {
          label: link.label,
          url: streamLinkVideos[index]
            ? `http://localhost:8080/static/${path.basename(streamLinkVideos[index].path)}`
            : link.url,
          image: streamLinkImages[index]
            ? `http://localhost:8080/static/${path.basename(streamLinkImages[index].path)}`
            : link.image || undefined,
          commentator: link.commentator || undefined,
          commentatorImage: streamLinkCommentatorImages[index]
            ? `http://localhost:8080/static/${path.basename(streamLinkCommentatorImages[index].path)}`
      ...
      const matchData: Partial<IMatch> = {
        ...
        mainCommentatorImage: mainCommentatorImage
          ? `http://localhost:8080/static/${path.basename(mainCommentatorImage.path)}`
          : body.mainCommentatorImage || undefined,
        secondaryCommentator: body.secondaryCommentator || undefined,
        secondaryCommentatorImage: secondaryCommentatorImage
          ? `http://localhost:8080/static/${path.basename(secondaryCommentatorImage.path)}`
          : body.secondaryCommentatorImage || undefined,
      };
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      for (const link of match.streamLinks) {
        if (link.url?.startsWith("http://localhost:8080/static/")) {
          oldFiles.push(path.join(__dirname, "../../assets/images", path.basename(link.url)));
        }
        if (link.image?.startsWith("http://localhost:8080/static/")) {
          oldFiles.push(path.join(__dirname, "../../assets/images", path.basename(link.image)));
        }
        if (link.commentatorImage?.startsWith("http://localhost:8080/static/")) {
          oldFiles.push(path.join(__dirname, "../../assets/images", path.basename(link.commentatorImage)));
        }
      }
      ...
      let newMainCommentatorImage = body.mainCommentatorImage === ""
        ? undefined
        : body.mainCommentatorImage || match.mainCommentatorImage;
      if (mainCommentatorImage) {
        if (match.mainCommentatorImage?.startsWith("http://localhost:8080/static/")) {
          oldFiles.push(path.join(__dirname, "../../assets/images", path.basename(match.mainCommentatorImage)));
        }
        newMainCommentatorImage = `http://localhost:8080/static/${path.basename(mainCommentatorImage.path)}`;
      }

      // Handle secondaryCommentatorImage
      let newSecondaryCommentatorImage = body.secondaryCommentatorImage === ""
        ? undefined
        : body.secondaryCommentatorImage || match.secondaryCommentatorImage;
      if (secondaryCommentatorImage) {
        if (match.secondaryCommentatorImage?.startsWith("http://localhost:8080/static/")) {
          oldFiles.push(path.join(__dirname, "../../assets/images", path.basename(match.secondaryCommentatorImage)));
        }
        newSecondaryCommentatorImage = `http://localhost:8080/static/${path.basename(secondaryCommentatorImage.path)}`;
      }
      ...
      const updateData: Partial<IMatch> = {
        ...
        scores: body.scores
          ...
          : match.scores,
        streamLinks: processedStreamLinks,
        isHot: body.isHot === "true" ? true : body.isHot === "false" ? false : match.isHot,
        mainCommentator: body.mainCommentator || match.mainCommentator,
        mainCommentatorImage: newMainCommentatorImage,
        secondaryCommentator: body.secondaryCommentator || match.secondaryCommentator,
        secondaryCommentatorImage: newSecondaryCommentatorImage,
      };

      // Update match
      const updatedMatch = await Match.findByIdAndUpdate(req.params.id, updateData, {
        new: true,
        runValidators: true,
      });
